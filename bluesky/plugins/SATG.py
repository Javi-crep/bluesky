...
def _write_gc_rel_scn(out_path: str, *,
    ...
    conflict_params: Dict with dpsi, dcpa, tlosh, dh, tlosv, spd values/ranges
    """
    rng = random.Random(seed) if seed is not None else random.Random()
    
    # Generate intruder aircraft ID
    intruder_acid = _gc_rel_next_acid()
    intruder_ac_type = rng.choice(intruder_types) if intruder_types else "A320"
    ...
    if target_data.get("create_target", False):
        ...
        target_latlon = _satg_safe_latlon2txt(target_lat, target_lon)
        content_lines.append(f"CRE {target_acid},{target_ac_type},{target_latlon},{target_hdg},{target_alt_ft},{target_spd}")
    ...
    # Generate CRECONFS command
    spd_param = f"spd={intruder_spd}" if intruder_spd else ""
    cmd_parts = [
        f"CRECONFS {intruder_acid} {target_acid}",
        f"dpsi={dpsi}",
        f"dcpa={dcpa}",
        f"tlosh={tlosh}",
        f"actype={intruder_ac_type}"
    ]
    
    if dh != 0:
        cmd_parts.append(f"dh={dh}")
    if tlosv != 0:
        cmd_parts.append(f"tlosv={tlosv}")
    if spd_param:
        cmd_parts.append(spd_param)
    
    content_lines.append(" ".join(cmd_parts))
    content_lines.append("")
...
def SATG_RC_CIRCLE(*argv):
    """SATG_RC_CIRCLE name n types center_lat center_lon radius_nm [mode] [altmode] [tcpa] [angle] [seed] [fl] [cas] [actypes] [overwrite]
    ...
    try:
        ...
        for i in range(n):
            ...
            if current_mode == "abs":
                ...
                # Generate target aircraft ID
                if append:
                    nmax_gcr = _scan_max_gcr_index(out_path)
                    target_acid = f"GCR{nmax_gcr+1}"
                else:
                    target_acid = "GCR1"
